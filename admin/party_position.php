...
$addError = $partyAddSuccess = $partyEditError = $partyEditSuccess = $partyDeleteError = $partyDeleteSuccess = '';
$editing_id = isset($_GET['edit']) ? (int)$_GET['edit'] : 0;

$posAddError = $posAddSucess = $posEditError = $posDeleteError = $posDeleteSucess = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_party'])) {
    ...
    if ($party_name === '') {
        ...
        try {
            ...
            if ($e->getCode() == 23000) {
                $partyEditError = 'Party name must be unique.';
            } else {
                $partyEditError = 'DB error: ' . htmlspecialchars($e->getMessage());
            }
            $editing_id = $party_id;
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_position'])) {
    ...
    if ($position_name === '') {
        ...
        try {
            ...
            if ($e->getCode() == 23000) {
                $posAddError = 'Position name must be unique.';
            } else {
                $posAddError = 'DB error: ' . htmlspecialchars($e->getMessage());
            }
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_position'])) {
    ...
    if ($position_name === '') {
        ...
        try {
            ...
            if ($e->getCode() == 23000) {
                $posEditError = 'Position name must be unique.';
            } else {
                $posEditError = 'DB error: ' . htmlspecialchars($e->getMessage());
            }
            $pos_editing_id = $position_id;
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_position'])) {
    ...
    try {
        ...
        if ($pos_editing_id == $position_id) $pos_editing_id = 0;
    } catch (PDOException $e) {
        ...
        <div class="management-box">
            ...
            <div class="add-party-form" style="max-width:400px;">
                <form method="post" autocomplete="off">
                    ...
                    <?php if ($partyAddSuccess): ?><div class="msg-success"><?= htmlspecialchars($partyAddSuccess) ?></div><?php endif; ?>
                    <div class="form-row">
            ...
            <?php endif; ?>
            <?php if ($partyDeleteError): ?><div class="msg-error"><?= htmlspecialchars($partyDeleteError) ?></div><?php endif; ?>
            <?php if ($partyDeleteSuccess): ?><div class="msg-success"><?= htmlspecialchars($partyDeleteSuccess) ?></div><?php endif; ?>
            <?php if ($partyEditError): ?><div class="msg-error"><?= htmlspecialchars($partyEditError) ?></div><?php endif; ?>
            <?php if ($partyEditSuccess): ?><div class="msg-success"><?= htmlspecialchars($partyEditSuccess) ?></div><?php endif; ?>
